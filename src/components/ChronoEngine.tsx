...
import { Target, Play, RotateCcw, BrainCircuit } from 'lucide-react';
import { useLedger } from '@/lib/useLedger';

type Phase = 'intro' | 'compression' | 'dilation' | 'results';

interface ChronoEngineProps {
  onComplete?: () => void;
  autoStart?: boolean;
  hideControls?: boolean;
}

export function ChronoEngine({ onComplete, autoStart = false, hideControls = false }: ChronoEngineProps) {
  const [phase, setPhase] = useState<Phase>(autoStart ? 'compression' : 'intro');
    ...
    <div className="w-full h-full flex flex-col items-center justify-center relative overflow-hidden">
      ...
      <div className={`absolute inset-0 flex flex-col items-center justify-center bg-zinc-950 transition-opacity duration-300 ${phase === 'compression' ? 'opacity-100 z-20' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className="absolute top-24 text-center z-30">
          <h2 className="text-red-500 font-mono tracking-widest uppercase text-sm mb-2">{hideControls ? 'Sprint' : 'Stage 1: Time Compression'}</h2>
